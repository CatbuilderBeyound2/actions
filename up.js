...
async function run() {
  try {
    ...
    const release_id =
      process.env.APP_VERSION_NUMBER ||
      core.getInput("release_id", { required: false });
    const release_name =
      process.env.GITHUB_SHA || "Automated Github Action Release";
    ...
    const [owner, repo] = core
      .getInput("release-repo", { required: true })
      .split("/");
    ...
    console.log(
      `Uploading ${path} to ${name} on release ${release_id} to ${owner}/${repo}`
    );
    ...
    const existingRelease = releases.data.filter(
      er => er.tag_name == release_id
    );
